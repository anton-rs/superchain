...
impl arbitrary::Arbitrary<'_> for AddressList {
    fn arbitrary(g: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        Ok(Self {
            address_manager: Address::from(g.arbitrary::<[u8; 20]>()?),
            l1_cross_domain_messenger_proxy: Address::from(g.arbitrary::<[u8; 20]>()?),
            l1_erc721_bridge_proxy: Address::from(g.arbitrary::<[u8; 20]>()?),
            l1_standard_bridge_proxy: Address::from(g.arbitrary::<[u8; 20]>()?),
            l2_output_oracle_proxy: Some(Address::from(g.arbitrary::<[u8; 20]>()?)),
            optimism_mintable_erc20_factory_proxy: Address::from(g.arbitrary::<[u8; 20]>()?),
            optimism_portal_proxy: Address::from(g.arbitrary::<[u8; 20]>()?),
            system_config_proxy: Address::from(g.arbitrary::<[u8; 20]>()?),
            system_config_owner: Address::from(g.arbitrary::<[u8; 20]>()?),
            proxy_admin: Address::from(g.arbitrary::<[u8; 20]>()?),
            proxy_admin_owner: Address::from(g.arbitrary::<[u8; 20]>()?),
            guardian: Address::from(g.arbitrary::<[u8; 20]>()?),
            anchor_state_registry_proxy: Some(Address::from(g.arbitrary::<[u8; 20]>()?)),
            delayed_weth_proxy: Some(Address::from(g.arbitrary::<[u8; 20]>()?)),
            dispute_game_factory_proxy: Some(Address::from(g.arbitrary::<[u8; 20]>()?)),
            fault_dispute_game: Some(Address::from(g.arbitrary::<[u8; 20]>()?)),
            mips: Some(Address::from(g.arbitrary::<[u8; 20]>()?)),
            permissioned_dispute_game: Some(Address::from(g.arbitrary::<[u8; 20]>()?)),
            preimage_oracle: Some(Address::from(g.arbitrary::<[u8; 20]>()?)),
            challenger: Some(Address::from(g.arbitrary::<[u8; 20]>()?)),
        })
    }
}
